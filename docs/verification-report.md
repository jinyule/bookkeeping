...
- `adb shell am instrument -w -r com.example.bookkeeping.test/androidx.test.runner.AndroidJUnitRunner`：通过，`OK (1 test)`。
- `./gradlew connectedDebugAndroidTest` 在 vivo 上会被系统安装器的外部来源/相同版本确认页拦截；已改用手工确认安装测试 APK 后直接运行 instrumentation runner。
...
- 真机覆盖安装和实测：
  ...
  - `adb shell am instrument -w -r com.example.bookkeeping.test/androidx.test.runner.AndroidJUnitRunner`：通过，`OK (1 test)`。测试改为使用 `am start -W` + `dumpsys window` 验证前台 Activity，避免 vivo/Compose 下 `rootInActiveWindow` 偶发为空。
...
- 脏数据清理：
  ...
  - 清理后多次复查账单数仍为 8 条；最终状态下 `enabled_accessibility_services` 包含 `com.example.bookkeeping/com.example.bookkeeping.service.BookkeepingAccessibilityService`，本 app 无障碍服务已启用且未继续产生垃圾记录。

## 真机验证

- 已安装包：`com.example.bookkeeping`，版本 `0.1.4`。
...
- 崩溃扫描：
  - 最近 logcat 未发现 `com.example.bookkeeping` 的 FATAL/ANR。
